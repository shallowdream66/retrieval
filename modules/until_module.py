...
class CrossEn(nn.Module):
    ...
    def forward(self, sim_matrix, temp = 2):
        # sim_matrix = sim_matrix * F.normalize(sim_matrix)
        # logpt = F.log_softmax(sim_matrix / temp, dim=-1)
...
class NetVLAD(nn.Module):
    def __init__(self, cluster_size, feature_size = 512, add_batch_norm = True):
        super().__init__()
        self.feature_size = feature_size
        self.cluster_size = cluster_size
        init_scaled = 1/math.sqrt(feature_size)
        self.clusters_1 = nn.Parameter(init_scaled * torch.randn(feature_size, cluster_size))
        self.clusters_2 = nn.Parameter(init_scaled * torch.randn(1, feature_size, cluster_size))
    ...
    def forward(self, x):
        ...
        assignment = torch.matmul(x, self.clusters_1)
...
class CrossAttention(nn.Module):
    def __init__(self, input_dim = 512, output_dim =512, input_q_dim = 512 ):
        super(CrossAttention, self).__init__()
        self.input_dim = input_dim
        self.output_dim = output_dim
        self.input_q_dim = input_q_dim
        self.query = nn.Linear(input_q_dim, output_dim)
        self.key = nn.Linear(input_dim, output_dim)
        self.value = nn.Linear(input_dim, output_dim)

    def forward(self, key_words, seq_features):
        bs = key_words.shape[0]
        num_key = key_words.shape[1]
        num_words = seq_features.shape[1]

        q = self.query(key_words)
        k = self.key(seq_features)
        v = self.value(seq_features)

        attn_score = torch.matmul(q, k.transpose(-2, -1)) / (self.output_dim ** 0.5)
...
class SelfAttention(nn.Module):
    def __init__(self, dim = 512, dropout_rate = 0.1):
    ...
    def forward(self, sequence):
        ...
        K = self.proj_k(sequence)
        Q = self.proj_q(sequence)
        V = self.proj_v(sequence)

        atten = nn.Softmax(dim =-1)(torch.bmm(Q, K.transpose(-2,- 1))) * self._norm_fact
...
class VAE(nn.Module):
    def __init__(self, dim = 512, nn_dim = 256, nn_dim2 = 128, latent_dim = 10):
        super(VAE, self).__init__()

        self.fc1 = nn.Linear(dim, nn_dim)
