...
from modules.until_module import PreTrainedModel, AllGather, CrossEn, NetVLAD, CrossAttention, SelfAttention, VAE
from modules.module_cross import CrossModel, CrossConfig, Transformer as TransformerClip

from modules.module_clip import CLIP, convert_weights
from modules.modeling import CLIP4ClipPreTrainedModel, show_log, update_attr, check_attr
from modules.co_transformer import Co_attention_block
...
class XCLIP(CLIP4ClipPreTrainedModel):
    def __init__(self, cross_config, clip_state_dict, task_config):
        ...
        self.global_mat_weight = nn.parameter.Parameter(torch.eye(embed_dim))
        self.action_mat_weight = nn.parameter.Parameter(torch.eye(embed_dim))

        # for fine-grained added constrast weights
        ...
        self.sim_para = nn.parameter.Parameter(torch.rand(1,1),requires_grad=True)
    ...
    def forward(self, input_ids, token_type_ids, attention_mask, keywords_ids, keywords_mask, video, video_mask=None):
        ...
        keywords_output = self.get_keywords_output(keywords_ids)

        if self.training:
            loss = 0.
            sim_matrix, *_tmp = self.get_similarity_logits(sequence_output, seq_features, visual_output, attention_mask,
                                        video_mask, keywords_output, keywords_mask, shaped=True, loose_type=self.loose_type)
    ...
    def get_keywords_output(self, keywords_ids, shaped = False):
        bs_pair = keywords_ids.size(0)
        _, keywords_output = self.clip.encode_text(keywords_ids, return_hidden = True)
        keywords_output = keywords_output.float()
    ...
    def get_visual_output(self, video, video_mask, shaped=False, video_frame=-1):
        ...
        visual_hidden = visual_hidden.view(bs_pair, -1, visual_hidden.size(-1)).float()
    ...
    def _loose_similarity(self, sequence_output, seq_features, visual_output, attention_mask, video_mask, keywords_output, keywords_mask, sim_header="meanP"):
        """
        ...
        """
        sequence_output, visual_output, seq_features = sequence_output.contiguous(), visual_output.contiguous(), seq_features.contiguous()
        ...
        if sim_header == "meanP":
            ...
            visual_output_original = visual_output
            visual_output = pack_padded_sequence(visual_output, torch.sum(video_mask, dim=-1).cpu(), batch_first=True, enforce_sorted=False)
        ...
        #local video feature
        # con_feature = torch.cat([visual_output, video_output.unsqueeze(1).repeat(1,12,1)], dim = -1)
        # con_feature = pack_padded_sequence(con_feature, torch.sum(video_mask, dim=-1).cpu(), batch_first=True, enforce_sorted=False)
        # con_feature, _ = self.lstm_frame(con_feature)
        # if self.training: self.lstm_frame.flatten_parameters()
        # con_feature, _ = pad_packed_sequence(con_feature, batch_first=True)
        # con_feature = con_feature + visual_output_original
        # con_feature = con_feature / con_feature.norm(dim = -1, keepdim = True)
        ...
        sentence_output = sequence_output / sequence_output.norm(dim=-1, keepdim=True)

        # local textual feature
        words_feature = self.cross(seq_features, keywords_output)
        words_feature = words_feature + seq_features
        words_feature = words_feature / words_feature.norm(dim = -1, keepdim = True)
        # words_feature = seq_features / seq_features.norm(dim=-1, keepdim=True)
        ...
        if self.training:
            ...
            words_feature = allgather(words_feature, self.task_config)
        ...
        # fine grained score
        fine_logits = logit_scale * self._attenion_over_fine_grained_sim_matrix(words_feature, frame_features)

        logits = video_sentence_logits*self.sim_para + fine_logits*(1-self.sim_para)
        #logits = video_sentence_logits + fine_logits
    ...
    def get_similarity_logits(self, sequence_output, seq_features, visual_output, attention_mask, video_mask, keywords_output, keywords_mask, shaped=False, loose_type=False):
        if shaped is False:
            attention_mask = attention_mask.view(-1, attention_mask.shape[-1])
            video_mask = video_mask.view(-1, video_mask.shape[-1])
        ...
        if loose_type:
            ...
            retrieve_logits = self._loose_similarity(sequence_output, seq_features, visual_output, attention_mask, video_mask, keywords_output, keywords_mask, sim_header=self.sim_header)
